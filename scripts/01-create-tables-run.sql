...
-- (Removed ALTER DATABASE that sets app.jwt_secret because managed DBs often disallow this)

-- Create custom types (use safe existence checks for older Postgres versions)
DO $$ BEGIN
...
CREATE POLICY IF NOT EXISTS "Users can view own firm" ON firms FOR SELECT USING (
    id IN (SELECT firm_id FROM profiles WHERE id = auth.uid())
);

-- Profiles: Users can see profiles from their firm
CREATE POLICY IF NOT EXISTS "Users can view firm profiles" ON profiles FOR SELECT USING (
    firm_id IN (SELECT firm_id FROM profiles WHERE id = auth.uid())
);

-- Clients: Users can see clients from their firm
CREATE POLICY IF NOT EXISTS "Users can view firm clients" ON clients FOR SELECT USING (
    firm_id IN (SELECT firm_id FROM profiles WHERE id = auth.uid())
);

CREATE POLICY IF NOT EXISTS "Users can insert firm clients" ON clients FOR INSERT WITH CHECK (
    firm_id IN (SELECT firm_id FROM profiles WHERE id = auth.uid())
);

CREATE POLICY IF NOT EXISTS "Users can update firm clients" ON clients FOR UPDATE USING (
    firm_id IN (SELECT firm_id FROM profiles WHERE id = auth.uid())
);

-- Cases: Users can see cases from their firm
CREATE POLICY IF NOT EXISTS "Users can view firm cases" ON cases FOR SELECT USING (
    firm_id IN (SELECT firm_id FROM profiles WHERE id = auth.uid())
);

CREATE POLICY IF NOT EXISTS "Users can insert firm cases" ON cases FOR INSERT WITH CHECK (
    firm_id IN (SELECT firm_id FROM profiles WHERE id = auth.uid())
);

CREATE POLICY IF NOT EXISTS "Users can update firm cases" ON cases FOR UPDATE USING (
    firm_id IN (SELECT firm_id FROM profiles WHERE id = auth.uid())
);

-- Similar policies for other tables...
CREATE POLICY IF NOT EXISTS "Users can view firm documents" ON documents FOR SELECT USING (
    firm_id IN (SELECT firm_id FROM profiles WHERE id = auth.uid())
);

CREATE POLICY IF NOT EXISTS "Users can view firm time entries" ON time_entries FOR SELECT USING (
    firm_id IN (SELECT firm_id FROM profiles WHERE id = auth.uid())
);

CREATE POLICY IF NOT EXISTS "Users can view firm calendar events" ON calendar_events FOR SELECT USING (
    firm_id IN (SELECT firm_id FROM profiles WHERE id = auth.uid())
);

CREATE POLICY IF NOT EXISTS "Users can view firm communications" ON communications FOR SELECT USING (
    firm_id IN (SELECT firm_id FROM profiles WHERE id = auth.uid())
);

CREATE POLICY IF NOT EXISTS "Users can view firm invoices" ON invoices FOR SELECT USING (
    firm_id IN (SELECT firm_id FROM profiles WHERE id = auth.uid())
);
